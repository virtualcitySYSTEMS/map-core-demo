...
export default async function setup() {
  ...
  await Promise.all([context.mapCollection2D, context.mapCollection3D].map(async (collection) => {
    await collection.setActiveMap([...collection][0].name);
    await collection.activeMap.gotoViewPoint(context.startingVP);
  }));

  context.synchronizer = new Synchronizer(context.mapCollection2D.activeMap, context.mapCollection3D.activeMap);
